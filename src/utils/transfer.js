...
export const getAllUser = (token, page, search) => {
  const URL = !search
    ? `${process.env.NEXT_PUBLIC_BACKEND_HOST}/user?limit=6&sort=firstName%20ASC&page=${page}`
    : `${process.env.NEXT_PUBLIC_BACKEND_HOST}/user?limit=6&search=${search}&sort=firstName%20ASC&page=${page}`;
